...
void main(void)
{
    ...
    P5IE |=  BIT5;                              // P1.1 interrupt enabled
    P5IES |= BIT5;                              //falling edge
    P5REN |= BIT5;                              // Enable resistor on SW2 (P1.1)
    P5OUT |= BIT5;                              //Pull up resistor on P1.1
    P5IFG &= ~BIT5;                             // P1.1 Interrupt Flag cleared
...
__interrupt void WDT_ISR (void)
{
    ...
    if (!(P5IN & BIT5))         //if button is pressed
    {
        ...
        switch(TA0CCR1)
        {                       //Linearly increment PWM logarithmically
            case 0:
                {
                    TA0CCR1 = 8;
                }
...
__interrupt void Port_5(void)
{
    ...
    WDTCTL = WDT_MDLY_32;      //watchdog gets 32ms delay
